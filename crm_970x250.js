...
// return new AssetDirectory();
  ...
  function Creative(dmo){
    ...
    var R, Platform, AssetDirectory, Settings, Analytics, AnalyticsContent, TweenMax, TweenLite, TimelineLite, TimelineMax, EventForwarding, Hammer/* [INSERT_PLUGIN_VARS] */;
    ...
    function init(wrapperID) {
      ...
      req( ['require'].concat(config.bundles.Rosetta), function() {
        ...
        AssetDirectory = req("platform/usweb/AssetDirectory");
...
updateCurrFloat();
    ...
    /**
     * Receives parameters and returns a constructed AssetDirectory URL
     * @return {string} The directory structure with a trailing / but without a leading /.
     *
     * @memberof AssetDirectory
     */
...
// console.log(getWidth());
     ...
     AssetDirectory.init = function (type, subdirectory) {
       ...
       var subDir = subdirectory;
          if (subDir === undefined) {
              subDir = platformProps.subdirectory;
          }
          if (subDir) {
              if (subDir.lastIndexOf("/") !== subDir.length - 1) {
                  subDir = subDir + "/";
              }
              appendage = appendage + subDir;
          }
          ...
          if (type === "shared") {
              appendage = appendage + "shared";
          } else if (type === "size" && width && height) {
              appendage = appendage + width + "x" + height;
          } else if (type === "evergreen" && width && height) {
              appendage = appendage + "evergreen/" + width + "x" + height;
              // console.log("Appendage", appendage);
          }
          if (appendage.length > 0 && appendage.lastIndexOf("/") !== appendage.length - 1) {
              appendage = appendage + "/";
          }
          return appendage;
      }

       AssetDirectory.get = AssetDirectory.init;
      ...
      var borders = {
        l: R.create("div").set({ width: "1px", height: height, backgroundColor: borderColor, left: 0, top: 0, zIndex: 1002, pointerEvents: "none", parentNode: stage }).render(),
        r: R.create("div").set({ width: "1px", height: height, backgroundColor: borderColor, right: 0, top: 0, zIndex: 1002, pointerEvents: "none", parentNode: stage }).render(),
        t: R.create("div").set({ width: width, height: "1px", backgroundColor: borderColor, left: 0, top: 0, zIndex: 1002, pointerEvents: "none", parentNode: stage }).render(),
        b: R.create("div").set({ width: width, height: "1px", backgroundColor: borderColor, left: 0, bottom: 0, zIndex: 1002, pointerEvents: "none", parentNode: stage }).render()
      };
      ...
      if (obj.cta_text) {
        ...
        // console.groupCollapsed("ctaText Data");
        // console.log("CTA obj left:", ctaText.left),
        // console.table(ctaText);
        // console.groupEnd("ctaText Data");

        var cta_text = R.create("div").set({
          css: {
            color: ctaText.color,
            //color: R.create("var").set({ name: "cta_text_color", defaultValue: ctaText.color, dataType: "Color", required: false, exposed: true }).render().value(),
      ...
      if (obj.preheader_text) {
        var preheader_text = R.create("div").set({
          css: {
            color: R.create("var").set({ name: "preheader_text_color", defaultValue: "#000000", dataType: "Color", required: false, exposed: true }).render().value(),
            ...
            verticalAlign: "middle",
            marginTop: 0,
            backgroundColor: R.create("var").set({ name: "preheader_text_bg_color", defaultValue: "", dataType: "Color", required: false, exposed: true }).render().value(),
            padding: R.create("var").set({ name: "preheader_text_padding", defaultValue: "", dataType: "String", required: false, exposed: true }).render().value(),
          ...
          attr: {
            id: "preheader_text",
            textContent: R.create("var").set({ name: "preheader_text", defaultValue: "", dataType: "String", required: false, exposed: true }).render().value()
          }
        });
      }
